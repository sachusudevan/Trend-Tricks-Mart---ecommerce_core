...
var KTDatatablesServerSide = function() {
    ...
    var initDatatable = function() {
        ...
        dt = $("#ecommerce-category-datatable").DataTable({
            searchDelay: 500,
            serverSide: true,
            responsive: true,
            processing: true,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        let edit_url = api_config.edit_url.replace('0', row.id.toString());
                        return `<div class="d-flex align-items-center">
                                    <a href="${row.image}" target="_blank" class="symbol symbol-50px">
                    ...
                    render: function (data, type, row) {
                        ...
                        return `
                                <div class="d-flex justify-content-end flex-shrink-0">
                                    ...
                                    <a href="javascript:void(0)" data-id=${row.id} data-kt-category-table-filter="delete_row" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
                                        <span class="svg-icon svg-icon-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    var handleStatusFilter = () => {
        const filterStatus = document.querySelector('[data-kt-ecommerce-category-filter="status"]');
    ...
    var initToggleToolbar = function() {
        ...
        const container = document.querySelector('#ecommerce-category-datatable');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.post(`${api_config.delete_records}`, { ids: row_ids, 'csrfmiddlewaretoken': `${api_config.csrfmiddlewaretoken}`, }, function(data, status, xhr) {
                        ...
                        if (data.status = 200) {
                            Swal.fire({
                                text: "You have deleted all selected category!.",
                                icon: "success",
                                buttonsStyling: false,
                                confirmButtonText: "Ok, got it!",
                                customClass: {
